...
(*env)->
CallVoidMethod(env, this
->vpn_service, method_id, (jint)code);
...
(*env)->
CallVoidMethod(env, this
->vpn_service, method_id, (jint)state);
success = !androidjni_exception_occurred(env);
...
this->sockets->
insert_last(this
->sockets, (void*)(intptr_t)fd);
return
bypass_single_socket((intptr_t)
fd, this);
}
this->sockets->
invoke_function(this
->sockets, (void*)bypass_single_socket,
this);
return
TRUE;
}
...
*,
private_charonservice_t *this, public_key_t
*pubkey)
{
JNIEnv *env;
jmethodID method_id;
...
JNI_METHOD(CharonVpnService, initializeCharon, jboolean,
    ...
    DBG1(DBG_DMN, "Starting IKE charon daemon (strongSwan "
    VERSION
    ", %s %s, %s)",
