...
(*env)->
CallVoidMethod(env, jsignature, method_id, this
->key);
if (
...
(*env)->
CallVoidMethod(env, jsignature, method_id, jdata
);
if (
androidjni_exception_occurred(env)
)
...
->ptr, 0, signature->len);
memcpy(signature
->ptr + (len - r.len), r.ptr, r.len);
memcpy(signature
->ptr + len + (len - s.len), s.ptr, s.len);
chunk_free(&encoded);
}
else
{
...
METHOD(private_key_t, get_keysize,
int,
...
METHOD(private_key_t, get_fingerprint, bool,
        private_private_key_t
*this,
cred_encoding_type_t type, chunk_t
*fp)
{
return this->pubkey->
get_fingerprint(this
->pubkey, type, fp);
}
