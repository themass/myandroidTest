...
static void deliver_plain(private_android_service_t *this,
    ...
    if (this->tunfd < 0) {    /* the TUN device is already closed */
...
static job_requeue_t handle_plain(private_android_service_t *this) {
    ...
    if (len < 0) {
        if (errno == EBADF) {    /* the TUN device got closed just before calling select(), retry */
            return JOB_REQUEUE_FAIR;
        }
        DBG1(DBG_DMN, "select on TUN device failed: %s", strerror(errno));
        return JOB_REQUEUE_NONE;
    }
    else if (len == 0) {    /* timeout, check again right away */
...
static bool setup_tun_device(private_android_service_t *this,
    ...
    if (this->tunfd > 0) {    /* close previously opened TUN device */
        close(this->tunfd);
    ...
    if (!already_registered) {
        ...
        lib->processor->queue_job(lib->processor,
                                  (job_t *) callback_job_create((callback_job_cb_t) handle_plain,
                                                                this,
                                                                NULL,
...
static bool setup_tun_device_without_dns(private_android_service_t *this) {
    ...
    this->lock->write_lock(this->lock);
    if (this->tunfd >
        0) {    /* close previously opened TUN device, this should always be the case */
        close(this->tunfd);
    }
    this->tunfd = tunfd;
    this->lock->unlock(this->lock);
...
static void close_tun_device(private_android_service_t *this) {
    ...
    this->lock->write_lock(this->lock);
    if (this->tunfd < 0) {    /* already closed (or never created) */
        this->lock->unlock(this->lock);
        return;
    }
    tunfd = this->tunfd;
...
->lock);
if (!
setup_tun_device(this, ike_sa, child_sa
))
{
DBG1(DBG_DMN,
...
if (!
setup_tun_device_without_dns(this)
)
{
DBG1(DBG_DMN,
...
static void add_auth_cfg_pw(private_android_service_t *this,
    ...
    if (byod) {    /* use EAP-TTLS if BYOD is enabled */
...
METHOD(android_service_t, destroy,
void,
