...
DBG2(DBG_LIB,
"builder: adding interface address %H", addr);

prefix = addr->get_family(addr) == AF_INET ? 32 : 128;
if (
snprintf(buf,
sizeof(buf), "%H", addr) >= sizeof(buf))
{
goto
failed;
}
...
if (!(*env)->
CallBooleanMethod(env, this
->builder, method_id, str, prefix))
{
goto
failed;
}
...
DBG2(DBG_LIB,
"builder: adding route %+H/%d", net, prefix);

if (
snprintf(buf,
sizeof(buf), "%+H", net) >= sizeof(buf))
{
goto
failed;
}
...
DBG2(DBG_LIB,
"builder: adding DNS server %H", dns);

if (
snprintf(buf,
sizeof(buf), "%H", dns) >= sizeof(buf))
{
goto
failed;
}
...
METHOD(vpnservice_builder_t, establish,
int,
private_vpnservice_builder_t *this
)
{
return
establish_internal(this,
"establish");
}

METHOD(vpnservice_builder_t, establish_no_dns,
int,
private_vpnservice_builder_t *this
)
{
...
METHOD(vpnservice_builder_t, destroy,
void,
private_vpnservice_builder_t *this
)
{
