...
public class ArtistFragment extends BaseLoaderFragment {
    ...
    private BaseRecyclerViewAdapter.OnLongClickListener onLongClick = new BaseRecyclerViewAdapter.OnLongClickListener() {
        ...
        public void onLongItemClick(int position) {
            mActionMode = ((AppCompatActivity) getActivity()).startSupportActionMode(Helper.getActionCallback(((MusicXMainActivity) getActivity()), getContext(), new Action() {
                @Override
                public void clear() {
                    if (mActionMode != null) {
    ...
    private BaseRecyclerViewAdapter.OnItemClickListener mOnClick = (position, view) -> {
        if (songListAdapter.isMultiselect()) {
            ...
            switch (view.getId()) {
                case R.id.item_view:
                    ((MusicXMainActivity) getActivity()).onSongSelected(songListAdapter.getSnapshot(), position);
                    break;
                case R.id.menu_button:
                    ...
                    }, ((MusicXMainActivity) getActivity()), view, getContext(), song);
                    break;
            }
        }
    };
    ...
    private View.OnClickListener mOnClickListener = v -> {
        switch (v.getId()) {
            case R.id.shuffle_fab:
                ((MusicXMainActivity) getActivity()).onShuffleRequested(songListAdapter.getSnapshot(), true);
                break;
        }
    };

    private LoaderManager.LoaderCallbacks<List<Album>> albumLoadersCallbacks = new LoaderManager.LoaderCallbacks<List<Album>>() {
    ...
    private void fragTransition(Album album, ImageView imageView, String transition) {
        Helper.setFragmentTransition(((MusicXMainActivity) getActivity()), ArtistFragment.this, AlbumFragment.newInstance(album), imageView, transition, "albumdetail");
    }
    ...
    protected void background() {
        ...
        loadTrak();
        if (((MusicXMainActivity) getActivity()) != null) {
            AppCompatActivity appCompatActivity = ((MusicXMainActivity) getActivity());
