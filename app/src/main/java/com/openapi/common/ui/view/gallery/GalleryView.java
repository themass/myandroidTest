package com.openapi.common.ui.view.gallery;
...
public class GalleryView extends ViewGroup {
    ...
    public void init(List<Integer> wList, List<Integer> hList, int spaceW, int spaceH, int loadMoreHeight) {
        ...
        this.imgWidthList.add(CommonUtil.getScreenWidth(mContext));
        ...
        for (int i = imgHeightList.size() - wList.size() - 1; i < childCount; i++) {
            //最后一行时，无论怎样都让它的值超出屏宽
            lineWidth = i >= imgWidthList.size() - 2 ? CommonUtil.getScreenWidth(mContext) : lineWidth;
            //计算每一行的固定高度，以便计算ViewGroup的高度，这里减去多余的一个分割线宽
            if (lineWidth + imgWidthList.get(i) - lineSpaceWidth >= CommonUtil.getScreenWidth(mContext)) {
                //每行子View的宽度相加的和大于屏宽时
                ...
                //计算每行的实际高度
                lineHeight = (CommonUtil.getScreenWidth(mContext) - (i - index) * lineSpaceWidth) / childWH;
    ...
    private void createLoadMore() {
        ...
        addView(mLoadMore);
        mLoadMore.getLayoutParams().width = CommonUtil.getScreenWidth(mContext);
        mLoadMore.getLayoutParams().height = loadMoreHeight;
        mLoadMore.setGravity(Gravity.CENTER);
        mLoadMore.setPadding(0, loadMoreHeight / 2 - CommonUtil.sp2px(mContext, 8), 0, 0);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int height = parentHeight - updateHeight == 0 ? (int) parentHeight - lineSpaceHeight :
                (int) parentHeight - lineSpaceHeight * (updateTimes) - (updateTimes - 1) *
                        loadMoreHeight - (updateTimes - 1) * (int) CommonUtil.dip2px(mContext, 2.0f);
        height = isLoadMoreEnable ? height : height - loadMoreHeight - lineSpaceHeight;
        //实际高度减去多余一条分割线高
        setMeasuredDimension((widthMode == MeasureSpec.EXACTLY) ? sizeWidth : CommonUtil.getScreenWidth(mContext),
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        int height = parentHeight - updateHeight == 0 ? 0 : //上几行的总高度
                (int) (parentHeight - lineSpaceHeight * (updateTimes - 1) - (updateTimes - 1) *
                        loadMoreHeight - updateHeight) - (updateTimes - 1) * (int) CommonUtil.dip2px(mContext, 2.0f);
        float lineHeight;//每行的实际高度
        int lineWidth = 0;//每行子View的宽度相加的和
        ...
        for (int i = index; i < imgWidthList.size(); i++) {
            ...
            lineWidth = i >= (isLoadMoreEnable ? imgWidthList.size() - 2
                    : imgWidthList.size() - 1) ? CommonUtil.getScreenWidth(mContext) : lineWidth;
            //还是计算每行的实际高度
            if (lineWidth + imgWidthList.get(i) - lineSpaceWidth >= CommonUtil.getScreenWidth(mContext)) {
                //每行子View的宽度相加的和大于屏宽时
                float childWH = 0;//初始化宽高比之和
                ...
                for (int j = index; j <= i; j++) {
                    if (getChildAt(j).getVisibility() != GONE) {
                        ...
                        int left = lineWidth;
                        int top = height;
                        int right = j == i ? CommonUtil.getScreenWidth(mContext) : left + childWidth;
