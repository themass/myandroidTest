...
public class FileUtils {
    public static String CANHE_PATH="sexfree2";
...
//        return context.getFilesDir().getAbsolutePath()+File.separator +filePath;
        ...
        if (!sdStatus.equals(Environment.MEDIA_MOUNTED)) {
            return App.getApplication().getFilesDir().getAbsolutePath() + File.separator + CANHE_PATH;
        } else {
            return File.separator + "sdcard" + File.separator + CANHE_PATH;
        }
    }

    public static boolean ensureFile() {
        File path = new File(CANHE_PATH);
        if (!path.exists()) {
            path.mkdirs();
        }
        return true;
    }
    public static void delShareFile(String fileName) {
        File file = new File(getWriteFilePath()+File.separator+CANHE_PATH, fileName);
        if (file.exists()) {
            file.delete();
            ensureFile();
        }
