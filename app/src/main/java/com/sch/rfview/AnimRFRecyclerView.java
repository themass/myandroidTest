...
public class AnimRFRecyclerView extends RecyclerView implements Runnable {
    ...
    public void setHeaderImage(ImageView headerImage) {
        ...
        headerImageHeight = com.sch.rfview.AnimRFRecyclerView.this.headerImage.getHeight();
        // 防止第一次拉伸的时候headerImage.getLayoutParams().height = 0
        if (headerImageHeight <= 0) {
            headerImageHeight = com.sch.rfview.AnimRFRecyclerView.this.headerImage.getLayoutParams().height;
        } else {
    ...
    public void setAdapter(Adapter adapter) {
        if (mHeaderViews.isEmpty() || headerImage == null && isEnable) {
            ...
            headerImage = new com.sch.rfview.AnimImageView(mContext);
    ...
    public void onScrollStateChanged(int state) {
        ...
        // 当前不滚动，且不是正在刷新或加载数据
        if (state == RecyclerView.SCROLL_STATE_IDLE && mLoadDataListener != null && !isLoadingData && canLoadMore) {
