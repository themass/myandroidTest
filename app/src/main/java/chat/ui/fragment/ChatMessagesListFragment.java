...
public class ChatMessagesListFragment extends BaseMessagesFragment
    ...
    protected InfoListVo<Message> loadData(Context context) throws Exception {
        Long sessionId = PreferenceUtils.getPrefLong(getActivity(), Constants.CHAT_SESSION,0);
        List<ChatLog> chatlogs = DBManager.getInstance().getDaoSession().getChatLogDao().queryBuilder().where(new WhereCondition.StringCondition("sessionId = "+sessionId)).orderDesc(ChatLogDao.Properties.Id).limit(30).list();
